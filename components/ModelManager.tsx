    ...
    <div className="glass p-6 rounded-2xl border border-cyan-500/20 flex flex-col gap-5">
      <div className="flex justify-between items-center">
        ...
        <span className={`text-[9px] font-mono px-2 py-0.5 rounded-full border ${
          status === 'success' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-slate-800 border-white/5 text-slate-500'
        }`}>
          {status === 'success' ? 'ENGINE: ACTIVE' : 'ENGINE: STANDBY'}
        </span>
      </div>
      ...
      <div className="relative group">
        ...
        <div className="border-2 border-dashed border-slate-700 group-hover:border-cyan-500/50 p-6 rounded-xl transition-all text-center bg-slate-800/20">
          <p className="text-xs text-slate-400 font-medium">Upload .onnx File</p>
      ...
      {status === 'loading' && (
        <div className="flex items-center justify-center gap-3 py-2">
           ...
           <span className="text-[10px] font-mono uppercase text-cyan-400">Warming up WebGL kernels...</span>
        </div>
      )}
